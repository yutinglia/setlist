...
- 可搜尋的頻道瀏覽，以及顯示最近 10 個頻道更新與 100 首收錄歌曲的近期更新頁
- 網格形式的影片瀏覽，以及歌曲詳情與資料庫摘要
- 英文、繁體中文與日文介面
...
Pull request 通過必要檢查並合併後，再替目前 `main` commit 建立 tag：
...
| `GET` | `/v1/channels` | 已追蹤頻道；可用 `q` 依名稱或頻道 ID 做字面搜尋 |
| `GET` | `/v1/updates/recent` | 最近更新的 10 個頻道與 100 首收錄歌曲 |
...
頻道瀏覽搜尋同樣會在送出表單後執行，並將字面比對的 `q` 保留在網址中。
`%` 與 `_` 會視為頻道名稱文字，而不是 SQL 萬用字元。
...
6. 精確中繼資料可以升級約略值；之後的稀疏觀察不會抹除較完整的快照或先前
   成功的歌單。
7. 只有高信心 YouTube 封鎖訊號（429、明確 bot／CAPTCHA 或 IP 封鎖）才會
   中止剩餘呼叫並保存冷卻時間，避免重啟繞過限制。單一影片的年齡、會員、
   私人或地區限制，以及缺乏其他封鎖證據的 HTTP 403，仍是有界限的單筆失敗。
...
9. 正式環境的 yt-dlp 會在可終止的子程序執行，並具有網路與整體操作期限。
   ...
   週期顯示為 stalled。
10. 後端映像包含與 yt-dlp 版本相符的 EJS 腳本及固定版本的 Deno runtime。
    新的留言快照會記錄有界限的擷取診斷（請求／回傳數、排序、深度、回覆政策、
    版本與可能截斷），但不會因此儲存更多留言內容。
